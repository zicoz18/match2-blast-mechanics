...
namespace Game.Core.BoardBase
{
	public class Cell : MonoBehaviour
	{
		...
		public List<Cell> GetNeighboursIncludingDiagonals()
		{
			List<Cell> neighboursIncludingDiagonals = new List<Cell>();
			foreach (Direction direction in (Direction[])Enum.GetValues(typeof(Direction)))
			{
				if (direction == Direction.None) continue;
				var neighbour = Board.GetNeighbourWithDirection(this, direction);
				if (neighbour != null) neighboursIncludingDiagonals.Add(neighbour);
			}
			return neighboursIncludingDiagonals;
		}
